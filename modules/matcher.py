...
_EXPAND_MAP = {
    ...
    "HbsAg" :     "Поверхностный антиген Hepatitis B",
    "Covid" :     {"SARS-COV", "SARS",  "coronavirus"}, 
}
    ...
    def load(self, model_dir: Path) -> bool:
        try:
            ...
            if not faiss_path.exists():
                self.load_error = f"FAISS index not found: {faiss_path}"
                return False
            if not meta_path.exists():
                self.load_error = f"Catalog meta not found: {meta_path}"
                return False
